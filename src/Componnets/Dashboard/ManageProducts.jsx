...
export default function ManageProducts() {
  const { data, isLoading, refetch } = useQuery(["products"], () =>
    axios.get(`https://car-parts-bangladesh.herokuapp.com/api/services`, {
      headers: {
  ...
  const deleteProduct = (id) => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        axios
          .delete(
            `https://car-parts-bangladesh.herokuapp.com/api/services/${id}`,
            {
              headers: {
                authorization: `Bearer ${localStorage.getItem("aceessToken")}`,
              },
                ...
                <tbody>
                  {data?.data?.map((order, index) => (
                    <tr key={order?._id}>
                      <td>{index + 1}</td>
                      <td>
                        <img className="w-12 h-12" src={order?.image} alt="" />
                      </td>
